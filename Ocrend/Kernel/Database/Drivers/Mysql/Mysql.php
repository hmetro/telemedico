...
class Mysql extends \mysqli implements Driver
{
     ...
     */
    public function update(string $table, array $e, $where = null, $limit = null)
    {
        if (sizeof($e) == 0) {
        ...
        if ($this->real_query($query
            ...
            return array('status' => true, 'logs' => $this->affected_rows);
        } else {
            return array('status' => false, 'logs' => $this->error);
        }
    }
     ...
     */
    public function delete(string $table, $where = null, $limit = null)
    {
        $del = $this->real_query("DELETE FROM $table " . (null != $where ? "WHERE $where" : ' ') . (null !== $limit ? "LIMIT $limit" : ''));
